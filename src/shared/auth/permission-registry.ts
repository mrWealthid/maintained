...
export const PLATFORM_MANAGEABLE_PERMISSION_DEFINITIONS =
  PERMISSION_DEFINITIONS.filter(
    (permission) => permission.scope === PERMISSION_SCOPE.platform,
  );

export const PLATFORM_MANAGEABLE_PERMISSION_KEYS =
  PLATFORM_MANAGEABLE_PERMISSION_DEFINITIONS.map(
    (permission) => permission.key,
  );

export function isPlatformManageablePermissionKey(
  value?: string | null,
...
const TECHNICIAN_BASE_PERMISSIONS = [
  PERMISSION.WORKSPACE_DASHBOARD_VIEW,
  PERMISSION.WORKSPACE_SWITCH,
  PERMISSION.SETTINGS_VIEW,
  PERMISSION.SETTINGS_SESSIONS_VIEW,
...
export const DEFAULT_USER_TYPE_PERMISSIONS: Record<USER_TYPE, PermissionKey[]> =
  {
    [USER_TYPE.tenant]: TENANT_BASE_PERMISSIONS,
    [USER_TYPE.technician]: TECHNICIAN_BASE_PERMISSIONS,
  };
