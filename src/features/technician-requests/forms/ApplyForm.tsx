...
const ApplyForm: FC<ApplyTicketFormProps> = ({ ticketRequest }) => {
  ...
  //   () =>
  //     watchCosts?.reduce((sum, item) => sum + (Number(item.amount) || 0), 0) ??
  //     0,
  //   [watchCosts]
  // );
  ...
  const handleAddCost = async () => {
    ...
    if ((watchCosts.length > 0 && valid) || watchCosts.length === 0) {
      append({ title: "", amount: 0 });
    }
  };

  function calcCost() {
    return watchCosts?.reduce(
      ...
      >
        <div className="flex flex-col gap-3">
          ...
          <section className="border  rounded-lg p-4">
            ...
            {watchCosts.length === 0 && (
              <small>Add a breakdown of cost required to complete gig.</small>
            )}
            ...
            <div className="flex flex-col gap-3">
              {fields.map((item, index) => (
                <div key={item.id} className="flex items-start gap-4 mb-2">
                  ...
                  >
                    <Trash strokeWidth={2} size={14} color="red" />
                  </button>
                </div>
              ))}
            </div>
                  ...
                  render={({ field }) => (
                    <>
                      ...
                      {errors.schedule?.endTime && (
                        <ErrorMessage
                          errorMsg={errors.schedule.endTime?.message ?? ""}
                        />
                      )}
                    </>
                  )}
