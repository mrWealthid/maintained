...
export function InteractiveTicketChat({
  ...
  const speakMessage = (text: string) => {
    ...
    for (const preference of voicePreferences) {
      selectedVoice = voices.find(
        (voice) => voice.name === preference || voice.name.includes(preference)
      );
  ...
  const showSummary = () => {
    ...
    // )?.label;
    const locationLabel = LOCATIONS.find((l) => l.value === values.area)?.label;
...
Would you like to submit this request?`;
  ...
  const handleSelectChange = (value: string) => {
    ...
    if (lastBotMessage?.type === "bot" && lastBotMessage.field) {
      ...
      handleUserResponse(value, lastBotMessage.field, selectedOption?.label);
    }
  };
      ...
      >
        {/* <SheetClose asChild>
					<Button
						variant='ghost'
						size='icon'
						className='absolute top-2 right-1 rounded-full p-2 text-muted-foreground hover:bg-muted'>
						<X className='w-6 h-6' />{' '}
					</Button>
				</SheetClose> */}
                  ...
                  {message.options && (
                    <div className="flex flex-wrap gap-2 mt-2">
                      {message.options.map((option) => (
                        ...
                        >
                          {option}
                          {isLoading && /submit/i.test(option) && (
                            <LoaderCircle
                              strokeWidth={1}
                              size={18}
                      ...
                      {selectedFiles.length > 0 && (
                        <div className="space-y-1">
                          {selectedFiles.map((file, index) => (
                            <div
                              key={index}
                              className="flex items-center justify-between p-2 bg-muted/40 dark:bg-muted rounded text-xs"
                            >
                              <div className="flex items-center space-x-2">
        ...
        <div className="border-t border-border p-4">
          <div className="flex space-x-2">
            ...
            messages[messages.length - 1]?.inputType === "textarea" ? (
              <Textarea
                value={currentInput || getValues(field)?.toString()}
                onChange={(e) => {
            ...
            {isSpeechSupported && (
              <Button
                ...
                className={`flex-shrink-0 ${
                  isListening
                    ? "bg-destructive text-destructive border-destructive/40 dark:bg-destructive dark:text-destructive"
          ...
          <div className="flex items-center justify-between mt-2">
            ...
            {isListening && (
              <div className="flex items-center space-x-1 text-xs text-destructive dark:text-destructive">
